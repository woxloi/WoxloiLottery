    ...
    fun registerCommands() {
        ...
        addCommand(
            SCommandObject()
                ...
                .addArgument(SCommandArgument().addAlias("番号"))
                .addRequiredPermission("woxloilottery.buy")
                .addExplanation("宝くじを購入する(好きな番号1~100000)")
        ...
        addCommand(
            SCommandObject()
                ...
                .addArgument(SCommandArgument().addAlias("start"))
                .addArgument(SCommandArgument().addAlias("tick"))
                .addArgument(SCommandArgument().addAlias("回数"))
                .addRequiredPermission("woxloilottery.broadcast")
                .addExplanation("宝くじの告知をする(停止中)")
                .setExecutor(BroadCommand(plugin))  // CreateCommandが正しく機能することを確認
        )

        addCommand(
            SCommandObject()
                .addArgument(SCommandArgument().addAllowedString("broadcast"))
                .addArgument(SCommandArgument().addAlias("stop"))
                .addRequiredPermission("woxloilottery.broadcast.stop")
                .addExplanation("宝くじの告知を止める")
                .setExecutor(BroadCommand(plugin))  // CreateCommandが正しく機能することを確認
        )
        //===========================
        //
